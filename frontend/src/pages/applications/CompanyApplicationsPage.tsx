...
const CompanyApplicationsPage: React.FC = () => {
  ...
  const loadApplications = async () => {
    try {
      ...
      // For companies, we'll use the search endpoint with company filter
      const result: PaginatedApplicationsResponse = await applicationService.searchApplications(
        { ...filters, companyId: user?.id }, // Filter by current company
        pagination
      );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        {/* Page Header */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900">Mission Applications</h1>
          <p className="mt-2 text-gray-600">
            Review and manage applications for your posted missions
          </p>
        </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th
                    className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                    onClick={() => handleSort('freelancer.user.firstName')}
                  >
                    Freelancer
                    {sortBy === 'freelancer.user.firstName' && (
                      <span className="ml-1">{sortOrder === 'asc' ? '↑' : '↓'}</span>
                    )}
                  </th>
                  <th
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        ...
                        <div className="text-sm text-gray-500">
                          Budget: {formatCurrency(application.mission?.budget || 0)}
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
